    ...
    <header className="bg-white shadow-sm border-b border-gray-100 sticky top-0 z-100">
      ...
      <div className="bg-green-800 text-white py-2 px-4">
        <div className="max-w-7xl mx-auto text-center text-sm">
          <p className="truncate">🎉 Free shipping on orders over ৳3000! | <strong>Daily Deals</strong> - Up to 20% off</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto py-3 px-4 sm:px-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
            <button
              className="lg:hidden text-gray-700 hover:text-green-800 transition-colors p-1"
            ...
            <div className="hidden lg:block relative" ref={categoryMenuRef}>
              <button
                ...
                className="flex items-center space-x-1 px-4 py-2 bg-green-800 text-white rounded-lg hover:bg-green-600 transition-colors text-sm sm:text-base"
              ...
              {isCategoryMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-64 py-2 z-50">
            ...
            <form onSubmit={handleSearch} className="flex-1">
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2 pl-12 pr-20 border border-gray-300 rounded-lg focus:outline-none focus:border-green-800 focus:ring-2 focus:ring-green-200 transition-all text-sm"
                />
                <FiSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                <button
                  type="submit"
                  className="absolute right-1 top-1/2 transform -translate-y-1/2 bg-green-800 text-white px-3 py-1.5 rounded-md hover:bg-green-600 transition-colors text-sm"
                >
                  Search
                </button>
              </div>
          ...
          <div className="flex items-center space-x-3 sm:space-x-4">
            {/* Search Icon - Mobile */}
            <button 
              className="md:hidden text-gray-700 hover:text-green-800 transition-colors p-1"
            ...
            {session?.user?.role !== "admin" && (
              <div className="relative">
                <button
                  className="text-gray-700 hover:text-green-800 transition-colors relative group p-1"
            ...
            <div className="relative" ref={accountMenuRef}>
              {!session ? (
                <button
                  onClick={() => signIn()}
                  className="text-gray-700 hover:text-green-800 transition-colors flex items-center space-x-1 group p-1"
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center space-x-2 text-gray-700 hover:text-green-800 transition-colors group"
                    aria-label="Account menu"
                  >
                    <div className="w-7 h-7 sm:w-8 sm:h-8 bg-gradient-to-br from-green-800 to-teal-600 rounded-full flex items-center justify-center text-white font-semibold text-xs sm:text-sm">
                  ...
                  {isAccountMenuOpen && (
                    <div className="absolute right-0 mt-2 w-48 sm:w-56 bg-white shadow-xl rounded-lg border border-gray-200 py-2 z-50">
                      ...
                      <div className="px-3 py-2 border-b border-gray-100">
                        ...
                        {session.user?.role && (
                          <span className="inline-block mt-1 px-2 py-0.5 bg-green-100 text-green-800 text-xs rounded-full capitalize">
                            {session.user.role}
                          </span>
                        )}
                      </div>
        ...
        <div className={`md:hidden mt-3 transition-all duration-300 ${isMobileSearchVisible ? 'block' : 'hidden'}`}>
          <form onSubmit={handleSearch} className="relative">
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full px-4 py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:outline-none focus:border-green-800 text-sm"
            />
            <FiSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
            <button
              type="submit"
              className="absolute right-1 top-1/2 transform -translate-y-1/2 bg-green-800 text-white px-3 py-1 rounded-md hover:bg-green-600 transition-colors text-sm"
            >
              Go
            </button>
          </form>
        ...
        <nav className="hidden lg:flex items-center justify-center space-x-6 mt-3">
          <button
            onClick={() => handleNavigation("/")}
            className="text-gray-700 hover:text-green-800 font-medium transition-colors flex items-center space-x-1 text-sm"
          >
            <FiHome size={14} />
            <span>Home</span>
          </button>
      ...
      >
        <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-green-800 text-white">
            ...
            <div className="max-w-[200px]">
              {session ? (
                <>
                  ...
                  <p className="text-xs text-green-100 truncate">{session.user?.email}</p>
                </>
              ) : (
                <>
                  <p className="font-medium text-sm">Welcome!</p>
                  <p className="text-xs text-green-100">Sign in for better experience</p>
                </>
              )}
            </div>
          </div>
        ...
        <nav className="p-4 overflow-y-auto h-full pb-32">
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {!session ? (
              <button
                ...
                className="col-span-2 bg-green-800 text-white py-2 px-3 rounded-lg font-medium hover:bg-green-600 transition-colors text-sm"
              >
                Sign In / Register
              </button>
            ) : (
          ...
          <ul className="space-y-0">
            <li>
              <button
                onClick={() => handleNavigation("/")}
                className="flex items-center w-full text-left py-2 px-3 rounded-lg text-gray-700 hover:bg-green-50 hover:text-green-800 transition-colors text-sm"
              >
            ...
            <li>
              <div className="py-0">
                <button
                  ...
                  className="flex items-center justify-between w-full text-left py-2 px-3 rounded-lg text-gray-700 hover:bg-green-50 hover:text-green-800 transition-colors text-sm"
                >
                  <div className="flex items-center">
            ...
            {session?.user?.role === "admin" && (
              <li key="admin">
                <button
                  onClick={() => handleNavigation("/admin")}
                  className="flex items-center w-full text-left py-2 px-3 rounded-lg text-green-800 hover:bg-green-50 transition-colors text-sm border-t border-gray-200 mt-2 pt-2"
                >
